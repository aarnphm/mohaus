"""Build a PEP 503 "simple" index for `mohaus` and `mohaus-mojo` artifacts.

CI passes `--dist <dir>` (full of *.whl + *.tar.gz from earlier matrix jobs)
and `--out <site>` (the GitHub Pages staging dir). We compute SHA-256 over
each artifact and emit per-project index pages plus a root listing, matching
the spec at https://peps.python.org/pep-0503/.

The script is idempotent — re-running it overwrites the staging directory in
place. Use `--prune` to wipe the staging tree before writing (CI does this
to keep page storage bounded by per-commit artifact count).
"""
...
PROJECT_NAMES = ("mohaus", "mohaus-mojo")
...
def detect_distribution(file_name: str) -> str | None:
  for pattern in (WHEEL_NAME_RE, SDIST_NAME_RE):
    ...
    if candidate in {normalize(n) for n in PROJECT_NAMES}:
      return candidate
  return None
...
def write_landing(out_dir: Path, commit: str, commit_url: str) -> None:
  (out_dir / "index.html").write_text(
    "<!DOCTYPE html>\n"
    "<html>\n"
    "  <head><title>mohaus development index</title></head>\n"
    "  <body>\n"
    "    <h1>mohaus development index</h1>\n"
    f'    <p>Built from commit <a href="{html.escape(commit_url)}">{html.escape(commit)}</a>.</p>\n'
    "    <pre>uv pip install mohaus --index https://aarnphm.github.io/mohaus/simple/</pre>\n"
    "    <pre>uv pip install 'mohaus[mojo]' --index https://aarnphm.github.io/mohaus/simple/</pre>\n"
    f'    <p><a href="simple/">PEP 503 index</a></p>\n'
    "  </body>\n"
    "</html>\n",
    encoding="utf-8",
  )
